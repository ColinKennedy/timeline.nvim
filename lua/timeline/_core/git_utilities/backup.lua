...
local _GROUP = vim.api.nvim_create_augroup("TimelineGitBackupGroup", { clear = true })
...
local function _initialize_root(root)
    ...
    terminal.run({"git", "init"}, {cwd=root})
    terminal.run({"git", "config", "--local", "user.email", "timeline.nvim@noemail.com"}, {cwd=root})
    terminal.run({"git", "config", "--local", "user.name", "timeline.nvim"}, {cwd=root})
end
...
local function _backup_file(root, buffer)
    ...
    local source_path = vim.fn.fnamemodify(vim.fn.bufname(vim.fn.bufnr()), ":p")
    local repository_path = filer.join_path({root, git_parser.get_repository_path(source_path)})
